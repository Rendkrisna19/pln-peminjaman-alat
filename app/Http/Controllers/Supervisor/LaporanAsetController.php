...
class LaporanAsetController extends Controller
{
    public function index(Request $request)
    {
        ...
        // Statistik Cepat untuk Dashboard Atas
        $stats = [
        ...
        return view('supervisor.laporan_aset.index', compact('aset', 'stats', 'search', 'kondisi'));
    }
    ...
    public function exportPdf(Request $request)
    {
        ...
        $pdf = Pdf::loadView('supervisor.laporan_aset.pdf', compact('data', 'kondisi'))
            ->setPaper('a4', 'portrait');

        return $pdf->download('Laporan_Kondisi_Aset_PLN_' . date('Ymd') . '.pdf');
    }

    public function exportExcel(Request $request)
    {
        return Excel::download(new AsetExport($request->kondisi), 'Data_Aset_PLN_' . date('Ymd') . '.xlsx');
    }
}
